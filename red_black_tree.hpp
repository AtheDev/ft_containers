...
namespace   ft
{
    ...
    class   Node
    {
        public:
            ...
            Node(T const & val): parent(NULL), left(NULL), right(NULL), value(val), color(BLACK) {}

            Node(Node const & cpy): parent(cpy.parent), left(cpy.left), right(cpy.right), value(cpy.value), color(cpy.color) {}
            ~Node() {}
